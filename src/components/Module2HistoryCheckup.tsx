...
import { HistoryCheckupData, PrescriptionItem, Cie10Item, InstitutionalMed } from '../types';
...
export const Module2HistoryCheckup: React.FC<Module2HistoryCheckupProps> = ({
  ...
  const addPrescriptionItem = (med?: InstitutionalMed) => {
    const newItem: PrescriptionItem = {
      ...
      marcaInstitucional: med ? med.brand : 'FABE',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Padecimiento, somatometría, exploración, diagnóstico CIE-10 y prescripción institucional FABE / ALMUS
            </p>
          </div>
        </div>
      ...
      <div className="p-5 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-sky-700 dark:text-sky-400">
            ...
            <span>Somatometría y Signos Vitales</span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          {imcInfo.alertControlled && (
            <span className="ml-auto text-[11px] font-semibold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800 flex items-center gap-1">
              ...
              IMC &gt; 25: Apto para prescripción de control de peso institucional
            </span>
          )}
        </div>
      </div>
      ...
      {/* Prescripción / Orden de Surtido (FABE y ALMUS) */}
      <div className="p-5 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-2.5">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-sky-700 dark:text-sky-400">
              ...
              <span>Orden de Surtido y Prescripción Institucional (FABE / ALMUS)</span>
            </div>
            <p className="text-[11px] text-slate-500">Priorizar marcas genéricas institucionales de farmacia</p>
          </div>
        ...
        {/* Quick Institutional Medication Search Bar */}
        <div className="relative">
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              <Search className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
              <input
                type="text"
                placeholder="Buscar medicamento institucional FABE / ALMUS por nombre o sustancia (ej. amoxicilina, paracetamol, omeprazol, ketorolaco)..."
                value={medSearch}
                onChange={(e) => {
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded text-white ${
                          med.brand === 'FABE' ? 'bg-emerald-600' : med.brand === 'ALMUS' ? 'bg-blue-600' : 'bg-slate-600'
                        }`}>
                          {med.brand}
                        </span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-2">
                  ...
                  {item.marcaInstitucional && (
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded text-white ${
                      item.marcaInstitucional === 'FABE' ? 'bg-emerald-600' : item.marcaInstitucional === 'ALMUS' ? 'bg-blue-600' : 'bg-slate-600'
                    }`}>
                      {item.marcaInstitucional}
                    </span>
                  )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div className="sm:col-span-2">
                  <FieldWithCopy
                    ...
                    placeholder="Ej. Paracetamol 500 mg Tabletas (FABE)"
                    required
                  />
                </div>
